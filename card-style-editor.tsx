import React from "react";
    ...
    type TemplateConfig = {
      name: string;
      description: string;
      config: Partial<CardStyleConfig> & { gradient?: boolean; neonGlow?: boolean; }; // Added gradient and neonGlow to config type
    };
    ...
    export const CardStyleEditor = () => {
      // Core card properties
      const [cardDimensions, setCardDimensions] = React.useState<CardDimensions>({ width: 400, height: 240 });
      const [backgroundColor, setBackgroundColor] = React.useState("#8B5CF6"); // Purple-500
      const [secondaryColor, setSecondaryColor] = React.useState("#EC4899"); // Pink-500
      const [cornerRadius, setCornerRadius] = React.useState(16);
      const [opacity, setOpacity] = React.useState(95); // Percentage 0-100
      
      // Enhanced visual effects
      const [selectedTemplate, setSelectedTemplate] = React.useState("glass");
      const [gradientType, setGradientType] = React.useState("linear");
      const [gradientAngle, setGradientAngle] = React.useState(135); // Degrees
      const [shadowIntensity, setShadowIntensity] = React.useState(20); // "Strength" not px
      const [shadowColor, setShadowColor] = React.useState("#000000");
      const [borderWidth, setBorderWidth] = React.useState(1); // px
      const [borderColor, setBorderColor] = React.useState("#FFFFFF");
      const [borderOpacity, setBorderOpacity] = React.useState(20); // Percentage 0-100
      const [blurIntensity, setBlurIntensity] = React.useState(16); // px for backdrop-filter
      const [glassEffect, setGlassEffect] = React.useState(true);
      
      // Animation and interaction
      const [animationEnabled, setAnimationEnabled] = React.useState(true);
      const [animationType, setAnimationType] = React.useState("float"); // 'float', 'pulse', 'glow'
      const [hoverEffects, setHoverEffects] = React.useState(true);
      
      // Content and layout
      const [textContent, setTextContent] = React.useState<TextContent>({
      ...
      const [contentLayout, setContentLayout] = React.useState("default"); // 'default', 'centered', 'image-left'
      const [iconStyle, setIconStyle] = React.useState("gradient"); // 'gradient', 'outline', 'filled'
      
      // Preview and export
      const [previewMode, setPreviewMode] = React.useState(false);
      const [previewScale, setPreviewScale] = React.useState(1); // Multiplier
      const [previewBackground, setPreviewBackground] = React.useState("dark"); // 'dark', 'light', 'transparent'
      const [previewDevice, setPreviewDevice] = React.useState("desktop"); // 'desktop', 'tablet', 'mobile'
      ...
      // Enhanced templates
      const templates = React.useMemo<Record<string, TemplateConfig>>(() => ({
        minimal: {
          name: "Minimal",
      ...
      const colorPresets = React.useMemo<Record<string, ColorPreset>>(() => ({
      ...
      const showToastMessage = React.useCallback((message: string, duration = 3000) => {
        addToast({
          title: message,
          timeout: duration
        });
      ...
      const getCardStyle = React.useMemo(() => {
        ...
        if (currentTemplateConfig.gradient) {
          ...
          switch (gradientType) {
            case "linear":
              finalBackgroundColor = `linear-gradient(${gradientDirectionStr}, ${backgroundColor}, ${secondaryColor})`;
              break;
            case "radial":
              finalBackgroundColor = `radial-gradient(${gradientDirectionStr}, ${backgroundColor}, ${secondaryColor})`;
              break;
            case "conic":
              finalBackgroundColor = `conic-gradient(${gradientDirectionStr}, ${backgroundColor}, ${secondaryColor}, ${backgroundColor})`;
              break;
            default:
        ...
        if (currentTemplateConfig.glassEffect && glassEffect) {
          ...
          if (currentTemplateConfig.gradient) {
            ...
            switch (gradientType) {
              case "linear":
                finalBackgroundColor = `linear-gradient(${gradientDirectionStr}, ${primaryGlass}, ${secondaryGlass})`;
                break;
              case "radial":
                finalBackgroundColor = `radial-gradient(circle at center, ${primaryGlass}, ${secondaryGlass})`;
                break;
              case "conic":
                finalBackgroundColor = `conic-gradient(from ${gradientAngle}deg at 50% 50%, ${primaryGlass}, ${secondaryGlass}, ${primaryGlass})`;
                break;
              default: 
        ...
        return {
          ...
          position: "relative" as const, // Ensure 'position' is treated as a literal type
          overflow: "hidden" as const    // Ensure 'overflow' is treated as a literal type
        };
      }, [
    ...
    @keyframes glow { 
      ...
      const exportTailwind = React.useCallback(() => {
      ...
      const applyTemplate = React.useCallback((templateKey: string) => {
      ...
      const saveCurrentStyle = React.useCallback(() => {
      ...
      const loadStyle = React.useCallback((styleToLoad: SavedStyle) => {
      ...
      const deleteStyle = React.useCallback((styleId: number) => {
        setSavedStyles(prev => prev.filter(s => s.id !== styleId));
        showToastMessage("Style deleted.");
      }, [showToastMessage]);
      
      const copyToClipboard = React.useCallback(async (content: string, type = "Code") => {
        try {
      ...
      const exportToCanvas = React.useCallback(() => {
        ...
        if (cardStyleObj.background.includes("gradient")) {
          let gradient;
          if (cardStyleObj.background.includes("linear-gradient")) {
            const angleRad = (gradientAngle-90) * Math.PI / 180; 
            const x1 = 0;
            const y1 = 0;
            const x2 = cardDimensions.width * Math.cos(angleRad);
            const y2 = cardDimensions.height * Math.sin(angleRad); // Corrected: should be cardDimensions.height for y2 in some calculations
            gradient = ctx.createLinearGradient(x1, y1, x2, cardDimensions.width * Math.sin(angleRad)); // A common pattern is to use width for both components of direction vector for angle based gradients
            ...
            gradient = ctx.createRadialGradient(
              ...
              cardDimensions.width/2, 
              cardDimensions.height/2, 
              Math.max(cardDimensions.width, cardDimensions.height)/2
            );
          }
      ...
      const getDevicePreviewStyle = React.useMemo(() => {
      ...
      React.useEffect(() => {
        ...
        if (storedStyles) {
          try {
            ...
            localStorage.removeItem("cardEditorSavedStylesV3"); // Clear corrupted data
          }
        }
      }, []);
      ...
      const ControlWrapper = ({ title, icon, children }: { title: string; icon: string; children: React.ReactNode }) => (
      ...
      const RangeInput = ({ label, value, onChange, min, max, step = 1, unit = "" }: { 
        label: string; 
        value: number; 
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900 text-white relative overflow-x-hidden">
          ...
          <div className="flex">
              ...
              <div className="p-5 space-y-5">
                ...
                <ControlWrapper title="Dimensions & Scale" icon="lucide:move">
                  <div className="grid grid-cols-2 gap-3">
                    <Input 
                      label="Width (px)" 
                      type="number" 
                      value={cardDimensions.width.toString()} 
                    ...
                    <Input 
                      label="Height (px)" 
                      type="number" 
                      value={cardDimensions.height.toString()} 
                ...
                <ControlWrapper title="Appearance & Effects" icon="lucide:zap">
                  <RangeInput 
                    ...
                    min={0} 
                    max={Math.min(cardDimensions.width, cardDimensions.height)/2} 
                    unit="px" 
                  />
                  <RangeInput 
                    label="Opacity" 
                ...
                <ControlWrapper title="Animation & Interaction" icon="lucide:play"> {/* Changed icon from zap to play for clarity */}
                  <div className="flex items-center justify-between p-2 bg-slate-700/30 rounded-md border border-slate-600/50">
                  ...
                  <div className="flex items-center justify-between p-2 bg-slate-700/30 rounded-md border border-slate-600/50">
                    ...
                    <Switch 
                      isSelected={hoverEffects} 
                      onValueChange={setHoverEffects}
                      size="sm"
                      color="secondary"
                    />
                  </div>
                ...
                <ControlWrapper title="Content & Layout" icon="lucide:type">
                  ...
                  <Select 
                    ...
                    value={contentLayout} 
                    onValueChange={setContentLayout}
                    className="bg-slate-700/50 border-slate-600 text-white"
                    size="sm"
                  >
                ...
                <ControlWrapper title="Save & Load Styles" icon="lucide:save">
                  <Input 
                    ...
                    onValueChange={setCurrentStyleName} 
                      ...
                      {savedStyles.map(style => (
                        <div key={style.id} className="flex items-center justify-between p-2 bg-slate-700/40 rounded-md border border-slate-600/40">
                          ...
                          <div className="flex space-x-1.5">
                            <Button 
                              onPress={() => loadStyle(style)} 
                              title="Load Style" 
                              isIconOnly
                              size="sm"
                              variant="light"
                              className="text-blue-400 hover:text-blue-300 min-w-0 w-8 h-8"
                            >
                            ...
                            <Button 
                              ...
                              size="sm"
                              variant="light"
                              className="text-red-400 hover:text-red-300 min-w-0 w-8 h-8"
                            >
                ...
                <ControlWrapper title="Export Options" icon="lucide:download">
                  ...
                  <Button 
                    onPress={() => copyToClipboard(
                      ...
                      exportFormat === "tailwind" ? exportTailwind() : 
                      exportJSON(), 
            ...
            <div className={`flex-1 flex flex-col items-center justify-center p-5 transition-colors duration-300 ${
              ...
              <div className="w-full max-w-4xl mb-4 flex items-center justify-between">
                ...
                <div className="flex items-center space-x-1 p-1 bg-slate-700/50 rounded-lg border border-slate-600">
                  {[
                    ...
                    <Button
                      key={item.device} 
                      onPress={() => setPreviewDevice(item.device)} 
              ...
              <div className={`flex items-center justify-center flex-1 w-full relative transition-all duration-300 ${showGrid ? "bg-grid-pattern" : ""} 
                             ${previewDevice !== "desktop" ? "overflow-auto custom-scrollbar-small" : ""}`}
                   style={previewDevice !== "desktop" ? getDevicePreviewStyle : {}}>
                {/* Ensure React.CSSProperties is used for style prop for type safety */}
                <div style={previewDevice !== "desktop" ? {...getCardStyle, transform: "scale(1)"} as React.CSSProperties : getCardStyle as React.CSSProperties} 
          ...
          <style jsx global>{`
            ...
            @keyframes glow { 
                0%, 100% { /* Initial shadow from getCardStyle */ } 
                50% { /* Enhanced shadow, defined in CSS output based on current colors */ } 
            }
